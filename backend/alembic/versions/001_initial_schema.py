...
def upgrade() -> None:
    ...
    op.create_index(
        "idx_connections_pair", "connections", ["source_item_id", "target_item_id"]
    )
    ...
    # The triple
    op.create_index(
        "idx_snapshots_triple", "snapshots", ["item_id", "context_id", "source_id"]
    )
    # Conflict detection: same (what, when)
    op.create_index("idx_snapshots_what_when", "snapshots", ["item_id", "context_id"])
    # Change detection: same (what, who says)
    op.create_index("idx_snapshots_what_who", "snapshots", ["item_id", "source_id"])
    ...
    op.create_table(
        ...
        sa.Column(
            "can_import", sa.Boolean, nullable=False, server_default=sa.text("false")
        ),
        sa.Column(
            "can_edit", sa.Boolean, nullable=False, server_default=sa.text("true")
        ),
    )
    op.create_index("idx_permissions_user", "permissions", ["user_id"])
    op.create_index("idx_permissions_scope", "permissions", ["scope_item_id"])
    op.create_index(
        "idx_permissions_user_scope",
        "permissions",
        ["user_id", "scope_item_id"],
        unique=True,
    )
