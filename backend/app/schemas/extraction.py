...
class NounIdentification(BaseModel):
    ...
    matched_type: str | None = None                               # type_config type (e.g., "door"), or None if unmatched
    qualifiers: dict[str, str] = Field(default_factory=dict)      # Narrowing attributes (e.g., {material: "hollow metal"})
...
class NounExtraction(BaseModel):
    ...
    attributed_elements: list[str] = Field(default_factory=list)   # Element item IDs (as strings)
...
class SectionExtraction(BaseModel):
    ...
    nouns: list[NounExtraction] = Field(default_factory=list)    # Per-noun extraction results
